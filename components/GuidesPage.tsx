...
const GuideCard: React.FC<{ guide: Guide }> = ({ guide }) => (
    <motion.div variants={fadeInUpItem}>
        <Link to={`/guides/${guide.slug}`} className="block group bg-brand-surface border border-brand-outline rounded-lg overflow-hidden no-underline hover:border-brand-accent transition-all duration-300 hover:-translate-y-1 hover:shadow-lg hover:shadow-brand-accent/10 h-full flex flex-col">
            <div className="w-full h-56 relative overflow-hidden">
                 <GuideLabel title={guide.title} className="group-hover:scale-105 transition-transform duration-700" />
            </div>
  ...
  return (
    ...
    >
        <div className="text-center mb-12">
            <h1 className="text-4xl sm:text-5xl font-serif font-bold tracking-tight text-white">Spirit Guides</h1>
